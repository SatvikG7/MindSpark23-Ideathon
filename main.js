...
const displayStructure = (structure) => {
    const structureContainer = document.querySelector("." + CLASSES.structureContainer);
...
const displayProblemStatements = (problemStatements) => {
    const problemStatementsContainer = document.querySelector("." + CLASSES.problemStatementsContainer);
    const getGlitchItems = (count = 5) => {
        return new Array(count).fill(0).map(() => Render.div([], { className: CLASSES.glitchItem }));
    }
    const getProblemContainer = (problem) => {
        return Render.component("p", { className: CLASSES.glitchTitle, innerText: problem })
    }
    if (!problemStatements.length) {
        const glitchContainer = Render.div(getGlitchItems(), { className: CLASSES.glitchContainer });
        const problem = getProblemContainer("PROBLEM STATEMENTS RELEASING SOON");
    ...
    for (const stmt of problemStatements) {
        ...
        const glitchContainer = Render.div(glitchItems, { className: CLASSES.glitchContainer });
